...
export function ViewerDashboard({ data }: { data: DashboardData }) {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Operations Summary</h1>
        <p className="text-sm text-muted-foreground">Read-only view — no actions available for this role</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <ExceptionQueue exceptions={data.exceptions} readOnly limit={5} />
      </div>

      <AiRecommendations initial={data.recommendations} readOnly autoFetch={false} />
    </div>
  );
}
