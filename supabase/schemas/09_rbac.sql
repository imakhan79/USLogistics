...
-- SECURITY DEFINER for the same reason as current_tenant_id(): this is
-- called from write policies on every table (and from profiles_update
-- itself), so it must bypass RLS on profiles to avoid recursive policy
-- evaluation.
